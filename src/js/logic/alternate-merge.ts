...
export async function setupAlternateMergeTool() {
  ...
  try {
    ...
    showAlert(
      'Erro',
      'Falha ao carregar um ou mais arquivos PDF. Eles podem estar corrompidos ou protegidos por senha.'
    );
    console.error(error);
  } finally {
    hideLoader();
...
export async function alternateMerge() {
  if (Object.keys(alternateMergeState.pdfDocs).length < 2) {
    showAlert(
      'Nenhum arquivo',
      'Por favor, carregue pelo menos dois arquivos PDF para alternar e misturar.'
    );
    return;
  }

  showLoader('Alternando e misturando páginas...');
  try {
    ...
    showAlert('Sucesso', 'PDFs foram misturados com sucesso!');
  } catch (e) {
    console.error('Alternate Merge error:', e);
    showAlert('Erro', 'Ocorreu um erro ao misturar os PDFs.');
  } finally {
    hideLoader();
  }
}

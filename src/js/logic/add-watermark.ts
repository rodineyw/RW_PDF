...
export async function addWatermark() {
  ...
  try {
    ...
    if (watermarkType === 'text') {
      ...
      if (!imageFile)
        throw new Error('Por favor, selecione uma imagem para a marca d\'água.');
      ...
      if (imageFile.type === 'image/png') {
        ...
        throw new Error(
          'Formato de imagem não suportado. Por favor, use PNG ou JPG para a marca d\'água.'
        );
      }
    }
    ...
    for (const page of pages) {
      ...
      if (watermarkType === 'text') {
        ...
        if (!text.trim())
          throw new Error('Por favor, insira texto para a marca d\'água.');
    ...
    showAlert(
      'Erro',
      e.message || 'Não foi possível adicionar a marca d\'água. Por favor, verifique seus inputs.'
    );
  } finally {
    hideLoader();
  }

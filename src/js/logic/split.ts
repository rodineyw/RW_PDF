...
async function renderVisualSelector() {
  ...
  showLoader('Renderizando pré-visualizações das páginas...');
  try {
    ...
    console.error('Erro ao renderizar pré-visualizações das páginas:', error);
    showAlert('Erro', 'Falha ao renderizar pré-visualizações das páginas.');
  ...
  try {
    ...
    switch (splitMode) {
      case 'range':
        ...
        if (!pageRangeInput) throw new Error('Por favor, insira um intervalo de páginas.');
      ...
      case 'even-odd':
        ...
        if (!choiceElement) throw new Error('Por favor, selecione páginas pares ou ímpares.');
    ...
    showAlert(
      'Erro',
      e.message || 'Falha ao dividir PDF. Por favor, verifique sua seleção.'
    );
  } finally {
    hideLoader();
  }
